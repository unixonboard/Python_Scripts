import sys
acctbalance=10000

uinpt= 10

match uinput:
    case 1:
        print('check balance:',acctbalance)
    case 2:
        withdraw=int(input('How much amount to withdraw'))
        acctbalance=acctbalance-withdraw
        print('balance:',acctbalance)
    case 3:
        deposit=int(input('How much amount to deposit as cash'))
        acctbalance=acctbalance+deposit
        print('balance:',acctbalance)
    case 4:
        deposit=int(input('How much amount to deposit as cheque'))
        acctbalance=acctbalance+deposit
        print('balance:',acctbalance)


# import sys
#
# option = my_list=sys.argv[1]
#
# accBalance=10000
#
#
# while option != '100':
#
#     if option=='0':
#         print(accBalance)
#     elif option=='1':
#         value_with_draw = int(input('value to get: '))
#         accBalance=accBalance-value_with_draw
#     elif option=='2':
#         value_deposit_cash = int(input('value to deposit: '))
#         accBalance=accBalance+value_deposit_cash
#     elif option=='3':
#         value_deposit_check = int(input('value to deposit: '))
#         accBalance=accBalance+value_deposit_check
#     else:
#         break
#     option = input('option: ')